...
namespace FragAssetFormats.Geometry.WavefrontOBJ;

/// <summary>
/// Importer for the Wavefront OBJ 3D file format.
/// </summary>
public sealed class ObjImporter : IModelImporter
{
	#region Types
	...
	public bool ImportSurfaceData(in ImporterContext _importCtx, Stream _resourceFileStream, out MeshSurfaceData? _outSurfaceData, string? _fileExtension = null) //TODO: Output a list of submeshes as individual meshes instead, or implement composite mesh type.
	{
		...
		{
			_importCtx.Logger.LogError("Cannot import OBJ model from write-only stream!");
			_outSurfaceData = null;
			return false;
		}
		...
		do
		{
			if (ReadLine(_resourceFileStream, ref buffers))
			{
				ParseLine(in _importCtx, in buffers, geometryData);
			}
		}
		while (!buffers.eof);
		...
		return _outSurfaceData.IsValid;
	}
	...
	private static bool ParseLine(in ImporterContext _importCtx, in Buffers _buffers, GeometryData _geometryData)
	{
		string txt;
		...
		switch (prefix)
		{
			...
			case 'v':
				...
				return type switch
				{
					'n' => ParseVector3(in _importCtx, _buffers, _geometryData.normals),   // Normal vector
					't' => ParseVector2(in _importCtx, _buffers, _geometryData.uvs),       // Texture coordinate
					_ => ParseVector3(in _importCtx, _buffers, _geometryData.positions),   // Position
				};
	...
	private static bool ParseVector3(in ImporterContext _importCtx, in Buffers _buffers, List<Vector3> _vectors)
	{
		...
		for (int i = 2; i < _buffers.charLength; ++i)
		{
			...
			if (_buffers.utf16[i] != ' ')
			{
				...
				if (float.TryParse(span, _importCtx.CultureInfo, out float value))
				{
	...
	private static bool ParseVector2(in ImporterContext _importCtx, in Buffers _buffers, List<Vector2> _vectors)
	{

	...
	private bool LoadImmediately(ResourceHandle _handle, ResourceHandle.FuncAssignResourceCallback _assignResourceCallback)
	{
		...
		switch (_handle.resourceType)
		{
			...
			case ResourceType.Texture:
				if ((success = ImageImporter.ImportImageData(this, _handle, out RawImageData? rawImageData, out TextureData? textureData) &&
					(rawImageData is not null || textureData is not null)) &&
					(success = TextureResource.CreateTexture(_handle, engine.GraphicsSystem.graphicsCore, rawImageData, textureData, out TextureResource? texture)))
				{

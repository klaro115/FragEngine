...
namespace FragEngine3.Resources
{
	...
	public sealed class ResourceHandle : IEquatable<ResourceHandle>
	{
		...
		public ResourceHandle(ResourceManager _resourceManager, string _resourceName, string _filePath, ResourceType _resourceType, List<string>? _dependencies, ResourceSource _resourceSource)
		{
		...
		public ResourceHandle(ResourceManager _resourceManager, string _resourceName, string _filePath, ResourceType _resourceType, List<string>? _dependencies, ResourceFileType _fileType, ResourceSource _resourceSource, ulong _fileOffset, ulong _fileSize)
		{
			resourceManager = _resourceManager;
			resourceName = _resourceName ?? throw new ArgumentNullException(nameof(_resourceName), "Resource name may not be null!");
			filePath = _filePath ?? throw new ArgumentNullException(nameof(_filePath), "Resource file path may not be null!");
		...
		public ResourceHandle(ResourceManager _resourceManager, ResourceHandleMetadata _metadata, ResourceFileHandle _fileHandle)
		{
		...
		private static readonly ResourceHandle none = new(null!, string.Empty, string.Empty, ResourceType.Unknown, null, ResourceFileType.None, ResourceSource.Runtime, 0, 0);
		...
		public ResourceFileHandle.ResourceInfo FileInfo => new(Key, fileOffset, fileSize);
		...
		public bool Equals(ResourceHandle? other) => string.CompareOrdinal(Key, other?.Key) == 0;

...
{
	public static class ResourceFileFinderOld
	{
		#region Methods
		...
		public static bool GetMetadataFromDataFilePath(string _dataFilePath, out ResourceFileData _outMetadata)
		{
			if (string.IsNullOrEmpty(_dataFilePath))
			{
				_outMetadata = ResourceFileData.None;
				return false;
			}
			...
			// Wait a second, this is not a data file! Parse it as a descriptor file, you moron:
			if (_dataFilePath.EndsWith(ResourceConstants.FILE_EXT_METADATA, StringComparison.InvariantCultureIgnoreCase))
			{
				return ResourceFileData.DeserializeFromFile(_dataFilePath, out _outMetadata);
			}
			...
			try
			{
				FileInfo fileInfo = new(_dataFilePath);
				fileSize = (ulong)fileInfo.Length;
			}
			catch (Exception ex)
			{
				Logger.Instance?.LogException($"Failed to retrieve file size of resource data file '{_dataFilePath}'!", ex);
				_outMetadata = ResourceFileData.None;
				return false;
			}
			...
			ResourceHandleData singleResMetadata = new()
			{

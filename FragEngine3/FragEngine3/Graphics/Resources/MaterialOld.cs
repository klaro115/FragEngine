...
namespace FragEngine3.Graphics.Resources
{
    ...
    public sealed class MaterialOld(ResourceHandle _handle, GraphicsCore _graphicsCore) : Resource(_handle)
	{
		#region Types
		...
		public int ResourceLayoutCount => resLayouts != null ? resLayouts.Length : 0;
		...
		public bool GetOrUpdatePipeline(GraphicsDrawContext _ctx, CameraContext _cameraCtx, out Pipeline _outPipeline, MeshVertexDataFlags _vertexDataFlags = MeshVertexDataFlags.BasicSurfaceData)
		{
			if (_ctx == null)
			{
			...
			//{
			//	dirtyFlags |= DirtyFlags.Output;		//TODO: Identify and handle output change events
			//}
			...
			if (IsDirty && !variant.UpdatePipeline(_cameraCtx, dirtyFlags))
			{
				_outPipeline = null!;
				return false;
			}
		...
		public bool GetResourceLayouts(int _resourceLayoutIndex, out ResourceLayout _outResLayout)
		{
			if (_resourceLayoutIndex >= 0 && _resourceLayoutIndex < ResourceLayoutCount)
			{
				_outResLayout = resLayouts[_resourceLayoutIndex];
				return true;
			}
			_outResLayout = null!;
			return false;
		...
		public static bool CreateMaterial(ResourceHandle _handle, GraphicsCore _graphicsCore, out MaterialOld? _outMaterial)
		{
			...
			if (_handle.IsLoaded)
			{
				_outMaterial = _handle.GetResource(false, false) as MaterialOld;
				return true;
			}

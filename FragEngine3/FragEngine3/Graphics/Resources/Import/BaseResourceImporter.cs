...
public abstract class BaseResourceImporter<TImporter> : IDisposable
	where TImporter : IGraphicsResourceImporter
{
	...
	protected bool TryGetResourceFile(ResourceHandle _handle, out ResourceFileHandle _outFileHandle)
	{
		...
		{
			logger.LogError("Resource handle for graphics resource import may not be null or invalid!");
			_outFileHandle = ResourceFileHandle.None;
			return false;
		}
		if (_handle.resourceManager is null || _handle.resourceManager.IsDisposed)
		{
			logger.LogError("Cannot load graphics resource using null or disposed resource manager!");
			_outFileHandle = ResourceFileHandle.None;
			return false;
		}
